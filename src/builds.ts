...
export class BuildsProvider implements vscode.TreeDataProvider<Build | Step | None> {
				...
				if (element.contextValue?.startsWith('build')) {
					try {
						...
						if (buildInfo && buildInfo.stages && buildInfo.stages.length > 0) {
							return buildInfo.stages.map(
								(stage) => new Stage(stage, buildInfo!.number)
							);
						} else {
							return [new None('No stages found')];
						}
					} catch (e) {
						console.log('drone-ci-extension could not get build stages', e);
	...
	async getBuildInfo(build: Build): Promise<string | undefined> {
		try {
			const info = await this.client.getBuild(
				this.data?.owner,
				this.data?.repo,
				build.number
			);
	...
	async approveBuildStage(stage: Stage) {
		try {
			await this.client.approveBuild(
				this.data?.owner,
				this.data?.repo,
				stage.build,
				stage.number
	...
	async declineBuildStage(stage: Stage) {
		try {
			await this.client.declineBuild(
				this.data?.owner,
				this.data?.repo,
				stage.build,
				stage.number
	...
	async restartBuild(build: Build) {
		try {
			...
			vscode.window.showWarningMessage(
				`Build ${build.number} was not restarted. ${error.message}`
			);
		}
	}
